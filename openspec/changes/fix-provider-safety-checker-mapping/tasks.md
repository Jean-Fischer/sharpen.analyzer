...
- [ ] 1.3 Implement a central mapping registry (single source of truth) for safety-checked fix provider → safety checker
- [ ] 1.4 Add validation to the registry to enforce one-to-one constraints (no missing mappings, no duplicates)

## 2. Integration Flow

- [ ] 2.1 Update the safety-checked fix-application pipeline to resolve the safety checker exclusively via the mapping registry
...
- [ ] 3.2 Add mapping entries for all existing safety-checked fix providers
...
- [ ] 4.1 Add unit tests that assert the mapping is complete (every safety-checked fix provider has exactly one checker)
