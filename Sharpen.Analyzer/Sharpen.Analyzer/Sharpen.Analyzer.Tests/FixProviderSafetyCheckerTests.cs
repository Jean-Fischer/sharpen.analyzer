...
public sealed class FixProviderSafetyCheckerTests
{
    ...
    {
        var (document, semanticModel) =
            CreateCSharpDocumentAndSemanticModel("class C { void M() { var x = new int[] { 1, 2, 3 }; } }");
        ...
        var result = checker.IsSafe(document.GetSyntaxTreeAsync(CancellationToken.None).GetAwaiter().GetResult()!,
            semanticModel, diagnostic, CancellationToken.None);
    ...
    public void CollectionExpressionSafetyChecker_ReturnsUnsafe_WhenDiagnosticIsNull()
    {
        ...
        var checker = new CollectionExpressionSafetyChecker();
        var result = checker.IsSafe(document.GetSyntaxTreeAsync(CancellationToken.None).GetAwaiter().GetResult()!,
            semanticModel, null!, CancellationToken.None);
    ...
    {
        var (document, semanticModel) =
            CreateCSharpDocumentAndSemanticModel("class C { void M() { var s = string.Format(\"{0}\", 1); } }");
        var diagnostic = Diagnostic.Create(
