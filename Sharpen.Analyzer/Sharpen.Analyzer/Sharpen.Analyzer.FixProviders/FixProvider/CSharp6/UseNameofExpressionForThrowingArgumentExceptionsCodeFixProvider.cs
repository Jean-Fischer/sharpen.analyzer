...
[ExportCodeFixProvider(LanguageNames.CSharp,
    Name = nameof(UseNameofExpressionForThrowingArgumentExceptionsCodeFixProvider))]
[Shared]
public sealed class UseNameofExpressionForThrowingArgumentExceptionsCodeFixProvider : CodeFixProvider
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        if (!(root.FindNode(diagnosticSpan, getInnermostNodeForTie: true) is ExpressionSyntax stringLiteral)) return;

        context.RegisterCodeFix(
            CodeAction.Create(
                "Use nameof",
                c => UseNameofAsync(context.Document, stringLiteral, c),
                "Use nameof"),
            diagnostic);
    }
    ...
    {
        if (!CSharp6SyntaxHelpers.TryGetStringLiteralValue(stringLiteralExpression, out var paramName)) return document;
