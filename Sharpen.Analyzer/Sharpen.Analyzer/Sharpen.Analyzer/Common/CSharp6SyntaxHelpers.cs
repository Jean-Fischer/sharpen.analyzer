...
public static class CSharp6SyntaxHelpers
{
    ...
    public static bool IsNameofExpression(ExpressionSyntax expression)
    {
        ...
        if (invocation.Expression is not IdentifierNameSyntax identifier) return false;

        return string.Equals(identifier.Identifier.ValueText, "nameof", StringComparison.Ordinal);
    }

    public static ITypeSymbol GetTypeSymbolOrNull(SemanticModel semanticModel, TypeSyntax typeSyntax,
        CancellationToken cancellationToken)
    {
        if (semanticModel == null || typeSyntax == null) return null;
