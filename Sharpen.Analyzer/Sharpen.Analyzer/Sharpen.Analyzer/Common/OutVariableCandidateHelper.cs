...
{
    public static bool IsCandidate(SemanticModel semanticModel, ArgumentSyntax outArgument,
        bool outArgumentCanBeDiscarded)
    {
        if (outArgument.Expression is not IdentifierNameSyntax outArgumentIdentifier) return false;
        ...
        var usagesOfTheLocalVariable = enclosingStatement
            ...
            .Where(identifier =>
                ...
                SymbolEqualityComparer.Default.Equals(semanticModel.GetSymbolInfo(identifier).Symbol,
                    localVariableSymbol))
            .ToList();
        ...
        var numberOfUsagesAfterOutArgument =
            usagesOfTheLocalVariable.Count(identifier => identifier.Identifier.Span.Start >= outArgumentTextSpan.End);
    ...
    private static VariableDeclaratorSyntax? GetLocalVariableDeclaratorForOutArgument(
        StatementSyntax enclosingStatement, string outVariableName)
    {
