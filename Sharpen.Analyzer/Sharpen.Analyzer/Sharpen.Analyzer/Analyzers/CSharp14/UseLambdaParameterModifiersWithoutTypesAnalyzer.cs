...
public sealed class UseLambdaParameterModifiersWithoutTypesAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeLambda(SyntaxNodeAnalysisContext context)
    {
        ...
        if (typeInfo.ConvertedType is not INamedTypeSymbol { TypeKind: TypeKind.Delegate })
        {
            return;
        }
        ...
        if (lambda.Parent is not EqualsValueClauseSyntax equalsValueClause)
        {
            return;
        }

        if (equalsValueClause.Parent is not VariableDeclaratorSyntax variableDeclarator)
        {
            return;
        }
