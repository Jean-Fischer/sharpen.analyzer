...
public sealed class UseFromEndIndexInObjectInitializersSafetyChecker : IFixProviderSafetyChecker
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (indexExpressionNode is not ExpressionSyntax indexExpression)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "index-expression-not-found");
        ...
        if (indexExpression.Parent is not ArgumentSyntax { Parent: BracketedArgumentListSyntax { Parent: ImplicitElementAccessSyntax } })
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "implicit-element-access-not-found");
        ...
        if (indexExpression is not BinaryExpressionSyntax { Left: MemberAccessExpressionSyntax memberAccess })
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "length-minus-one-not-found");
        ...
        if (lengthTargetType is not IArrayTypeSymbol)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "length-target-not-array");
