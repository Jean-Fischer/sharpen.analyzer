...
public sealed class ImplicitSpanConversionsSafetyChecker : IFixProviderSafetyChecker
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (node is not InvocationExpressionSyntax asSpanInvocation)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "node-not-invocation");

        if (asSpanInvocation.Expression is not MemberAccessExpressionSyntax { Name.Identifier.ValueText: "AsSpan" })
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "not-as-span");

        if (asSpanInvocation.ArgumentList.Arguments.Count != 0)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "as-span-has-args");

        if (asSpanInvocation.Parent is not ArgumentSyntax argument)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "not-argument");

        if (argument.Parent is not ArgumentListSyntax argumentList)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "no-argument-list");

        if (argumentList.Parent is not InvocationExpressionSyntax outerInvocation)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "no-outer-invocation");
        ...
        if (asSpanSymbol is null || asSpanSymbol.Name != "AsSpan" || asSpanSymbol.ContainingType?.ToDisplayString() != "System.MemoryExtensions")
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "not-bcl-as-span");
        ...
        if (beforeSymbol is null)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "outer-symbol-null");
        ...
        if (speculativeSymbol is null)
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "speculative-symbol-null");

        if (!SymbolEqualityComparer.Default.Equals(beforeSymbol, speculativeSymbol))
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, reasonId: "overload-changed");
