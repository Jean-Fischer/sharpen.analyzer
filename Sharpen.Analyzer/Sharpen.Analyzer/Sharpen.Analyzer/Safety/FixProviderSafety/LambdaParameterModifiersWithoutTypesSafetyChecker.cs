...
public sealed class LambdaParameterModifiersWithoutTypesSafetyChecker : IFixProviderSafetyChecker
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, "csharp_version", "C# 14 or above is required.");
        }
        ...
        {
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, "node_not_found", "Expected a lambda parameter list.");
        }

        if (parameterList.Parent is not ParenthesizedLambdaExpressionSyntax lambda)
        {
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, "not_parenthesized_lambda", "Expected a parenthesized lambda.");
        }
        ...
        {
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, "not_target_typed", "Lambda is not target-typed to a delegate.");
        }
        ...
        if (!hasCandidate)
        {
            return FixProviderSafetyResult.Unsafe(FixProviderSafetyStage.Local, "no_candidate", "No parameter has both modifiers and an explicit type.");
        }
